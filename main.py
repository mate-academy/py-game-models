        ...
        for skill in skills:
            ...
            if not Skill.objects.filter(name=skill_name).exists():
                ...
                Skill.objects.create(
                    name=skill_name, bonus=skill_bonus, race_id=race.id
                )

        if guild:
            if not Guild.objects.filter(name=guild).exists():
                guild_info = info["guild"]["description"]
                Guild.objects.create(name=guild, description=guild_info)

            guild = Guild.objects.get(name=guild)

        if not Player.objects.filter(nickname=player).exists():
            Player.objects.create(
                nickname=player, email=email, bio=bio,
                race_id=race.id, guild=guild
            )
