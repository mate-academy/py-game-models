...
def main() -> None:
    with open("players.json", "r") as file:
        ...
        for nickname, i in data.items():
            race = Race.objects.get_or_create(name= i["race"]["name"], description=i["race"]["description"])
            for skill in i["race"]["skills"]:
                Skill.objects.get_or_create(name=skill["name"], bonus=skill["bonus"], race=race)
            guild = None
            if i["guild"] is not None:
                guild = Guild.objects.get_or_create(name=i["guild"]["name"], description=i["guild"]["description"])
